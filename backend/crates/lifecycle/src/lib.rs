...
impl LifecycleManager {
    ...
    pub fn validate_transition(from: AccountStatus, to: AccountStatus) -> Result<(), TransitionError> {
        match (from, to) {
            // Initial path
...
mod tests {
    ...
    fn test_valid_transitions() {
        assert!(LifecycleManager::validate_transition(AccountStatus::Ingested, AccountStatus::Assigned).is_ok());
        assert!(LifecycleManager::validate_transition(AccountStatus::Active, AccountStatus::Recovered).is_ok());
    }

    #[test]
    fn test_invalid_transitions() {
        // Cannot jump from Ingested to Recovered directly
        assert!(LifecycleManager::validate_transition(AccountStatus::Ingested, AccountStatus::Recovered).is_err());
        // Cannot re-open a Closed account
        assert!(LifecycleManager::validate_transition(AccountStatus::Closed, AccountStatus::Active).is_err());
    }
}
