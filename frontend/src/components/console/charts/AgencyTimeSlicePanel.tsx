...
const AgencyTimeSlicePanel = ({ isOpen, onClose, timeIndex, agencyData, isStableMode }: AgencyTimeSlicePanelProps) => {
    ...
    const chartData = agencies.map(agency => {
        ...
        // Logic to match DCABehaviorHeatmap's fullscreen view:
        // It displays the LAST 60 data points (timeSteps = 60).
        // The timeIndex (0-59) corresponds to that window.

        const timeSteps = 60;
        const recent = history.slice(-timeSteps);

        // Calculate padding just like the heatmap does
        const paddingCount = Math.max(0, timeSteps - recent.length);

        // If clicked on padding (empty space), no data
        if (timeIndex < paddingCount) return null;

        // Map visual index to array index in 'recent'
        const actualIndex = timeIndex - paddingCount;
        const point = recent[actualIndex];
        ...
        return {
            ...
            Active: point.active,
            Efficiency: (point.recovered + point.escalated) > 0
